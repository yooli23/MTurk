...
from config.settings import mturk as config
from config.settings import environments, worker_requirements

from botocore.exceptions import ClientError
from botocore.exceptions import ProfileNotFound

region_name = 'us-east-1'
aws_profile_name = 'eval_mturk'
client = None
...
def calculate_mturk_cost(payment_opt):
    ...
    if payment_opt['type'] == 'reward':
        ...
        total_cost = payment_opt['num_total_assignments'] * payment_opt['bonus']
    return total_cost
...
def create_hit_with_hit_type(hit_type_id, task_id: str):
    """
    Creates the actual HIT given the type and page to direct clients to.
    """
    complete_url = config['page_url'] + task_id
    ...
    question_data_struture = (
        ...
        '</ExternalQuestion>'
        ''.format(amazon_ext_url, page_url, config['frame_height'])
    )

    client = get_mturk_client(config['is_sandbox'])

    # Create the HIT
    response = client.create_hit_with_hit_type(
        HITTypeId=hit_type_id,
        MaxAssignments=1,
        LifetimeInSeconds=config['life_time'],
